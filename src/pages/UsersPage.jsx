...
export default function UsersPage() {
  ...
  const staffCount = users.filter(u => ['superadmin','stv_admin','inhaber','mitarbeiter'].includes(u.role)).length;
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto">
      <div className="pb-2 border-b border-border/40">
        ...
        <p className="text-muted-foreground mt-1">Alle registrierten Benutzer verwalten.</p>
      </div>

      {/* Bento Summary Tiles */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="bg-card/40 backdrop-blur-sm border-primary/20 hover:border-primary/40 transition-all group">
          <CardContent className="pt-5 pb-4 px-5 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Gesamt</p>
              <p className="text-3xl font-black mt-1">{loading ? <Skeleton className="h-9 w-12" /> : users.length}</p>
            </div>
        ...
        <Card className="bg-card/40 backdrop-blur-sm border-success/20 hover:border-success/40 transition-all group">
          <CardContent className="pt-5 pb-4 px-5 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Mitarbeiter</p>
              <p className="text-3xl font-black text-success mt-1">{loading ? <Skeleton className="h-9 w-12" /> : staffCount}</p>
            </div>
        ...
        <Card className="bg-card/40 backdrop-blur-sm border-destructive/20 hover:border-destructive/40 transition-all group">
          <CardContent className="pt-5 pb-4 px-5 flex items-center justify-between">
            <div>
              <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Gesperrt</p>
              <p className="text-3xl font-black text-destructive mt-1">{loading ? <Skeleton className="h-9 w-12" /> : blockedCount}</p>
            </div>
