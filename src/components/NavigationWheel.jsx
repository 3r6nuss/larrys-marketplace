...
export default function NavigationWheel() {
 ...
 return (
 <div 
 className={`fixed inset-0 z-[100] flex items-center justify-center transition-all duration-150 ${
 isOpen ? 'opacity-100 bg-background/80 visible' : 'opacity-0 bg-background/0 invisible pointer-events-none'
 }`}
 onContextMenu={(e) => e.preventDefault()}
 >
 <div 
 className={`relative w-[400px] h-[400px] transition-all duration-150 ease-out ${
 ...
 className={`cursor-pointer transition-all duration-150 stroke-background stroke-[4px] ${
 ...
 className={`absolute flex flex-col items-center justify-center w-24 h-24 -translate-x-1/2 -translate-y-1/2 transition-all duration-150 ${isHovered ? 'scale-110' : ''}`}
 ...
 <div className={`absolute bottom-10 text-muted-foreground text-sm font-medium transition-all duration-200 delay-150 ${
